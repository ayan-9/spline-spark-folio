...
-- Drop ALL existing policies on contact_messages to clean slate
...
DROP POLICY IF EXISTS "Only admins can update message status" ON public.contact_messages;

-- Create new admin-only policies for contact messages
CREATE POLICY "Admin only contact messages access" 
ON public.contact_messages 
FOR SELECT 
USING (public.has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admin only contact messages update" 
ON public.contact_messages 
FOR UPDATE 
USING (public.has_role(auth.uid(), 'admin'::app_role));